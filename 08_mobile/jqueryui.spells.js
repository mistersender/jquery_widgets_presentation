 ...
 $.widget("mf.spell", {
  ...
  _add_events: function(){
   var self = this,
       ...
       power = 0;
   self.element
    .on("mousedown.spell touchstart.spell", ".spell-container__wand", function(e){
     self.element.addClass("is-casting");
     power_interval = setInterval(function(){
      power++
     }, 20);
    })
    .on("mouseup.spell mouseout.spell touchend.spell", ".spell-container__wand", function(e){
     if(power){
  ...
  destroy: function(){
   ...
   self.element.removeClass("spell-container spell-container--" + self.options.spellname.replace(" ", "-", "all"));
